    ...
    return vim.lsp.buf.format {
        ...
        filter = function(client)
            local have_nls = package.loaded["null-ls"] and
                    ...
                    vim.bo[vim.api.nvim_get_current_buf()].filetype, "NULL_LS_FORMATTING") > 0)
...
local function substitute(cmd)
    ...
    cmd = cmd:gsub("$moduleName", vim.fn
        .substitute(vim.fn.substitute(vim.fn.fnamemodify(vim.fn.expand "%:r", ":~:."), "/", ".", "g"), "\\", ".", "g"))
    cmd = cmd:gsub("#", vim.fn.expand "#")
...
function _G.run_code()
    ...
    if supportedFiletypes[fileExtension] then
        ...
        if #choices == 0 then
            vim.notify("It doesn't contain any command", vim.log.levels.WARN, {
                title = "Code Runner"
            })
        ...
        vim.notify("The filetype isn't included in the list", vim.log.levels.WARN, {
            title = "Code Runner"
        })
    end
end
