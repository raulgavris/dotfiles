-- ============================================================================
-- Workspace system: VS Code-style multi-root workspaces
--
-- Stores workspace definitions as JSON in stdpath("data")/workspaces/.
-- Each workspace lists folders (repos). Opening a workspace:
--   - creates a symlink directory aggregating all repos
--   - cd's into it so Neo-tree shows all repos side by side
--   - find/grep search across all repos
-- ============================================================================
...
local function read_workspace(name)
  ...
  if not json_ok or not data or type(data.folders) ~= "table" then
...
local function get_folders(name)
  ...
  if not data or not data.folders then return nil end
  return data.folders
end
...
local function setup_symlink_dir(name, dirs)
  ...
  for _, dir in ipairs(dirs) do
    local expanded = strip_slash(expand(dir))
...
function M.get_dirs()
  if not M.active_dirs then return nil end
  local expanded = {}
...
function M.open(name)
  ...
  vim.cmd("Neotree show dir=" .. vim.fn.fnameescape(ws_dir))
...
function M.select()
  ...
  vim.ui.select(names, {
    ...
    format_item = function(name)
      local folders = get_folders(name)
      if not folders then return name end
      local repos = {}
      for _, dir in ipairs(folders) do
...
function M.add_folder(name, path)
  ...
  local expanded = strip_slash(expand(path))
  ...
  if M.active_name == name then M.open(name) end
end

--- Open a directory picker (Snacks) and call callback with the selected path
local function pick_folder(callback)
  ...
  for _, line in ipairs(lines) do
    line = line:gsub("/+$", "") -- strip trailing slash
    if line ~= "" then
      table.insert(items, { text = "~/" .. line, file = home .. "/" .. line })
    end
  end
  Snacks.picker.pick({
    title = "Select Folder",
    ...
    confirm = function(picker, item)
      ...
      if item and item.file then
        vim.schedule(function() callback(item.file) end)
      end
    end,
  })
end

--- Prompt to add a folder via directory picker, optionally creating a new workspace
function M.add_folder_interactive()
  local names = M.list()

  -- Build choices: existing workspaces + "New workspace..." option
  local choices = {}
  for _, n in ipairs(names) do
    table.insert(choices, n)
  end
  table.insert(choices, "+ New Workspace...")
    ...
    if choice == "+ New Workspace..." then
      vim.ui.input({ prompt = "New workspace name: " }, function(name)
        if name and name ~= "" then
          M.create(name)
          pick_folder(function(path) M.add_folder(name, path) end)
        end
      end)
    else
      pick_folder(function(path) M.add_folder(choice, path) end)
    end
  end)
end
  ...
  vim.ui.select(data.folders, {
    ...
    if choice then
      ...
      for _, dir in ipairs(data.folders) do
        if dir ~= choice then table.insert(new_folders, dir) end
      end
...
function M.info()
  if M.active_name then
    ...
    table.insert(lines, "Config: " .. ws_file(M.active_name))
...
-- Snacks picker wrappers: search across workspace dirs

function M.find_files()
  local dirs = M.get_dirs()
  if dirs then
    Snacks.picker.files({ dirs = dirs, follow = true })
  else
    Snacks.picker.files()
  end
end
...
function M.live_grep()
  local dirs = M.get_dirs()
  if dirs then
    Snacks.picker.grep({ dirs = dirs, follow = true })
  else
    Snacks.picker.grep()
  end
end
...
function M.grep_word()
  local dirs = M.get_dirs()
  if dirs then
    Snacks.picker.grep_word({ dirs = dirs, follow = true })
  else
...
vim.api.nvim_create_user_command("WorkspaceSelect", function() M.select() end,
  { desc = "Select and open a workspace" })

vim.api.nvim_create_user_command("WorkspaceCreate", function(opts) M.create(opts.args) end,
  { nargs = 1, desc = "Create a new workspace" })

vim.api.nvim_create_user_command("WorkspaceOpen", function(opts) M.open(opts.args) end,
  { nargs = 1, desc = "Open a workspace by name", complete = complete_workspace_names })
...
vim.api.nvim_create_user_command("WorkspaceAdd", function(opts)
  ...
  nargs = "+", desc = "Add a folder to a workspace",
  complete = function(arg_lead, cmd_line)
    local args = vim.split(cmd_line, " ", { trimempty = true })
    if #args <= 2 then return complete_workspace_names() end
...
vim.api.nvim_create_user_command("WorkspaceDelete", function(opts) M.delete(opts.args) end,
  { nargs = 1, desc = "Delete a workspace", complete = complete_workspace_names })

vim.api.nvim_create_user_command("WorkspaceClose", function() M.close() end,
  { desc = "Close active workspace" })

vim.api.nvim_create_user_command("WorkspaceInfo", function() M.info() end,
  { desc = "Show active workspace info" })
